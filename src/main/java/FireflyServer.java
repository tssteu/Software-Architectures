...
public class FireflyServer extends FireflyServiceGrpc.FireflyServiceImplBase {
    ...
    public void updatePhase(PhaseRequest request, io.grpc.stub.StreamObserver<Empty> responseObserver) {
        ...
        synchronized(phases) { phases.put(indexKey, phase); }
    ...
    public int getUpper(int index, int size) { return (index - 1 + size) % size; }

    public int getLower(int index, int size) { return (index + 1) % size; }

    public int getLeft(int index, int size) { return (index - 1 + size) % size; }

    public int getRight(int index, int size) { return (index + 1) % size; }
